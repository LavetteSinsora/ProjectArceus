"""
exp_004_1 evaluation — per-env greedy/sampling rollout across the four envs.

Usage:
    uv run python -m JEPA.experiments.exp_004_1_four_envs.eval \\
        --checkpoint <path> --env ls20 --episodes 50
    uv run python -m JEPA.experiments.exp_004_1_four_envs.eval \\
        --checkpoint <path> --env all  --episodes 50
"""
...
def eval_episodes(env_name: str, env, encoder, policy, n_episodes: int,
    ...
    for ep in range(n_episodes):
        ...
        while True:
            ...
            with torch.no_grad():
                queries = (encoder.perceiver.get_initial_queries(1, device)
                           if h_t is None else h_t.detach())
                h_current, _, _ = encoder(frame_t, queries)
                action_idx, _, _ = policy.act(h_current.squeeze(0),
                                              env.available_actions)
            assert action_idx < env_n_actions, (
                f"action_idx={action_idx} ≥ env.n_actions={env_n_actions} in "
                f"{env_name!r}"
            )
        ...
        if (ep + 1) % 10 == 0:
            print(f"  [{env_name}] ep {ep+1}/{n_episodes}: "
                  f"len={steps} completed={completed}")
    return {
        "env": env_name,
...
def main():
    ...
    parser.add_argument("--env",
                        choices=["ls20", "tu93", "re86", "g50t", "all"],
                        default="all")
    ...
    envs_to_eval = list(cfg.env_names) if args.env == "all" else [args.env]
