...
metrics.md §1.2, 1.3, 1.4@eval, 1.5, 2.1, 2.2, 4.3@eval, 6.2, 6.3.
...
Note: exp_003_4 removed self-attention among latents in the Perceiver
resampler, so metrics §3.1 (latent self-attn row JSD) no longer apply.
The §1.3 post-SA latent probe is captured via cross_attn output instead.
"""
...
def run_eval(
    ...
    # Register a forward hook that captures the round-0 cross_attn *output*
    # (post-cross-attn latents at round 0). exp_003_4 has no self-attn among
    # latents, so the round-0 cross_attn output is the closest analogue of
    # the post-SA probe used in earlier experiments.
    post_sa_cap = {"out": None}

    def _hook(_mod, _inp, out):
        # _CrossAttentionBlock.forward returns (queries, attn_w); take queries.
        post_sa_cap["out"] = out[0].detach() if isinstance(out, tuple) else out.detach()
    h_handle = encoder.perceiver.rounds[0].cross_attn.register_forward_hook(_hook)
