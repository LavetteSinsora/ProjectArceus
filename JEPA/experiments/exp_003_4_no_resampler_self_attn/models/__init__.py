...
def load_models(cfg, device: torch.device):
    """
    Construct all trainable modules for exp_003_4 (no resampler self-attn).

    Returns (encoder, state_predictor, action_predictor, action_embed, policy, baseline).
    """
    encoder = Encoder(
        d_model=cfg.d_model,
        d_color=cfg.d_color,
    ...
    return encoder, state_predictor, action_predictor, action_embed, policy, baseline
