...
const PERC_CA_CH  = 32;   // cross-attn cell height (4 rows × 32 ≈ bar height)

// Build one round row: [input bars] → [cross-attn] → [output bars]
// exp_003_4: self-attn among latents removed; cross-attn output is the round output.
...
function _refreshPerceiverPanel() {
  ...
  // exp_003_4: rounds are cross-attn only, so r{i}-out is the cross-attn output.
  const groups = {
