...
|---|---|---|---|
| **13_1** `exp_013_1_rnd_icm` | RND novelty computed *inside* ICM's inverse-dynamics φ-space; leaky predictor; φ frozen after warm-up | **composition** (ICM = the representation; one reward) | built, runs; high seed-variance on ls20 L1 |
| **13_2** `exp_013_2_rnd_icm_additive` | `w·norm(ICM-fwd-error) + (1−w)·norm(RND-on-φ)`, w=0.5 | **additive** (two rewards summed) | **DEAD** — see issue (4) |
| **13_4** `exp_013_4_disagreement` | Plan2Explore-style **ensemble disagreement** (variance of K forward models predicting a FROZEN-RANDOM φ(s')) | non-redundant signal; no ICM/φ-freeze | built, smoke-passes; not yet run |
| **13_5 (D)** `exp_013_5_lookahead` | **MCTS-organized 1-step lookahead-softmax**, actor-free: act by `softmax(standardize(nov(φ̂'_a)+γV(φ̂'_a))/τ)`; V_int trained on REAL returns | **structural fix for the entropy collapse** (no policy gradient → no phantom advantage); model used for decision only | built, smoke-passes |
