"""
Debug episode runner for exp_003_0_normalized_latent_jepa.

Per-timestep data collected:
  patch_embeddings:        (16, 128) — SA-output patch embeddings
  per_patch_stats:         list[16] — norm/mean/std/cos-sim vs prev per patch
  embedding_summary:       dict — pairwise cos-sim, eff-rank, dead-dims etc.
  encoder_sa_block1/2:     (16, 16) — SA attention averaged over heads (block 0/1)
  latent_vectors:          (4, 128) — h_t from recurrent Perceiver
  latent_norms:            [4]
  perceiver_cross_attn_r0/r1: (4, 16) — cross-attn avg over heads per round
  perceiver_self_attn_r0/r1:  (4, 4)  — self-attn avg over heads per round
  ode_trajectory:          list of (4, 128) per ODE step (start → final)
  h_target_latents:        (4, 128) — h_{t+1} target
  per_latent_pred_error:   [4] — MSE per latent
  action_probs, action_entropy, flow_loss

Usage (standalone test):
    cd "Code Repo"
    uv run python JEPA/experiments/exp_003_0_normalized_latent_jepa/debug_runner.py \\
        JEPA/experiments/exp_003_0_normalized_latent_jepa/checkpoints/step_001200_final.pt
"""
...
def run_debug_episode(checkpoint_path: str,
    ...
    encoder, predictor, action_embed, policy, _baseline = load_models(cfg, device)
    encoder.load_state_dict(ckpt["encoder"])
    predictor.load_state_dict(ckpt["predictor"])
    action_embed.load_state_dict(ckpt["action_embed"])
    policy.load_state_dict(ckpt["policy"])
    encoder.eval(); predictor.eval(); action_embed.eval(); policy.eval()
...
if __name__ == "__main__":
    _ckpt = (sys.argv[1] if len(sys.argv) > 1
             else "JEPA/experiments/exp_003_0_normalized_latent_jepa/checkpoints/step_001200_final.pt")
