...
## A — frozen-φ RND+leak (`exp_013_1_rnd_icm --phi-mode frozen`)
...
datum motivating a stronger leak. (See also `exp_014_1_rnd_saturation_diagnosis` for the μ
...
## B — RND on ICM-φ (`exp_013_1_rnd_icm`, phi_mode=icm)
...
1. **[HIGH] Hard guard: never freeze φ while `holdout_inv_acc < 1.5×chance`.** re86 froze a 0.22 φ
   ...
   ```python
   # exp_013_1_rnd_icm/trainer.py, in the freeze block
...
## C — additive (`exp_013_2_rnd_icm_additive`)
...
## D — lookahead (`exp_013_5_lookahead`)
