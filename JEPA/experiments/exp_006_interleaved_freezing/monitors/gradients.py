...
def compute_source_decomposition(
    ...
    # Only differentiate w.r.t. params that currently require grad — frozen-side
    # params (e.g. during a FreezeScheduler phase) would otherwise make
    # autograd.grad raise. Frozen positions surface as None → 0 norm in dashboard.
    active_mask = [p.requires_grad for p in upstream_params]
    active_params = [p for p, m in zip(upstream_params, active_mask) if m]

    def _grad(loss, retain):
        if not active_params or not loss.requires_grad:
            return [None] * len(upstream_params)
        active_grads = torch.autograd.grad(
            loss, active_params, retain_graph=retain, allow_unused=True,
        )
        # Re-expand into full upstream_params order, inserting None at frozen slots.
        out_grads: list = []
        it = iter(active_grads)
        for m in active_mask:
            out_grads.append(next(it) if m else None)
        return out_grads

    g_state    = _grad(L_state_m,         retain=True)
    g_via_Ht   = _grad(L_action_via_Ht,   retain=True)
    g_via_Htp1 = _grad(L_action_via_Htp1, retain=False)
