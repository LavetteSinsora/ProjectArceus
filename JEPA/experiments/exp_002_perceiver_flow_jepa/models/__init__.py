...
CAPABILITIES: dict = {
    "has_encoder_attention": True,       # Stage-1 ViT SA attention maps
    "has_perceiver_attention": True,     # 4-latent × 16-patch cross-attention
    "has_patch_embeddings": False,       # No individual patch embeddings exposed
    "has_latent_vectors": True,          # 4 latents per timestep
    "has_flow_matching": True,           # tau-loss breakdown available
    "n_latents": 4,
    "n_patches": 16,
...
def load_models(cfg, device: torch.device):
    """Instantiate all models from a Config object."""
    encoder = Encoder(
        d_model=cfg.d_model,
        d_color=cfg.d_color,
