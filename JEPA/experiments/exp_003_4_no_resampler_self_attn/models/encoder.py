"""
Exp-003-4 Encoder: same as exp_003_2 minus self-attention among latents.

Diff vs exp_003_0/exp_003_2:
  - `_SelfAttentionAmongLatents` is removed entirely.
  - `_PerceiverRound` is cross-attention only.
  - PerceiverResampler is otherwise identical: separate weights per round,
    output LayerNorm at the recurrent boundary.

Hypothesis: cross-attn already pools the same 16-patch context for every
latent, so SA among the 4 latents may either be redundant (training metrics
match exp_003_2) or load-bearing for diversity (eff_rank / pairwise cossim
worsen). This experiment tests which.
"""
...
# ── 2D RoPE (unchanged from exp_003_0) ────────────────────────────────────────
...
class _PerceiverRound(nn.Module):
    """One round of Perceiver: cross-attention only (no SA among latents)."""
...
class PerceiverResampler(nn.Module):
    ...
    Each round is cross-attention only; self-attention among latents is removed.
    """
...
class Encoder(nn.Module):
    """
    Full exp-003-4 encoder.

    stage1: patch embedding + 2 SA blocks with 2D RoPE → (B, 16, d_model)
    stage2: PerceiverResampler (cross-attn only, separate round weights, output_norm)
            → (B, n_latents, d_model)
    """
