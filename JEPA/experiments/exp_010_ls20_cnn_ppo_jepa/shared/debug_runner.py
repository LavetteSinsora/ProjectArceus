"""Shared dashboard episode runner for the exp_010 family.

Each sub-experiment's `debug_runner.py` re-exports `run_debug_episode` from
here. The main JEPA dashboard (JEPA/dashboard/debug_runner.run_debug_episode)
prefers a per-experiment runner when one exists, so this is what plays an
exp_010 CNN+PPO / JEPA checkpoint on the real LS20 game in the dashboard.

The payload mirrors the keys the dashboard frontend reads, with the JEPA/ViT-
specific capability flags set False (so the ViT-only cards hide cleanly). What
we *do* surface per step: the rendered frame, the chosen action, the full
action-probability vector, the critic value, and the terminal/reward flags.
"""
...
from JEPA.shared.env_wrapper import make_env, resolve_dashboard_env, short_env_name  # noqa: E402
from JEPA.experiments.exp_010_ls20_cnn_ppo_jepa.shared.model import ActorCritic  # noqa: E402
from JEPA.experiments.exp_010_ls20_cnn_ppo_jepa.shared.config_base import Config  # noqa: E402
from JEPA.experiments.exp_010_ls20_cnn_ppo_jepa.shared.device import get_device  # noqa: E402

# ARC-AGI 16-colour palette (indices 0–15) — same table the main dashboard uses.
ARC_COLORS_RGB = [
...
def run_debug_episode(checkpoint_path: str, env_name: str | None = None,
    ...
    cfg_gid = f"{cfg.env_name}"  # short name; resolve_dashboard_env accepts short+full
