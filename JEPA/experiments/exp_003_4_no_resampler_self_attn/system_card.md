# System Card — exp_003_4_no_resampler_self_attn

| Field | Value |
|-------|-------|
| **Experiment ID** | `exp_003_4_no_resampler_self_attn` |
| **Status** | Active |
| **Parent experiment** | `exp_003_4_no_resampler_self_attn` |
...
This experiment is **identical to `exp_003_4_no_resampler_self_attn` except that the Perceiver resampler no longer performs self-attention among the 4 latents**. Each Perceiver round is now cross-attention only: latents attend to the 16 patch tokens, the FFN runs, and that is the round's output (previously each round also ran a self-attention block over the 4 latents before producing its output). The hypothesis is that cross-attention already pools the same 16-patch context for every latent, so SA among 4 latents may be redundant. The competing concern is that without SA, the 4 latents have no interaction channel at all and may drift toward duplicated representations. Removing self-attn isolates this single architectural change so its effect on representational health (effective rank, pairwise cosine similarity, `H_t·H_{t+1}` cossim) can be measured directly against the exp_003_2 baseline on the existing dashboard. Everything else (action predictor, no EMA target, three-path encoder gradient, `λ_state=λ_action=0.5`, replay buffer storing raw next-frames) is unchanged.
...
frame (64×64 uint8)
  ...
  │  Round 1: Cross-Attn only      │  (separate weights per round)
  │  output_norm (LayerNorm)       │  (no self-attn among latents)
  │  → h_t (B, 4, 128)            │
  └────────────────────────────────┘
       │
       ├──────────────────────────► Policy MLP → action
...
| Heads | 4 per cross attention |
| Self-attn among latents | **removed in exp_003_4** (was present in exp_003_0–003_3) |

**Per-round computation (cross-attention only):**
...
h = h + FFN(norm(h))           # pre-norm FFN — round output (no SA step)

# After all rounds:
h = output_norm(h)             # (B, 4, 128) — unit-scale recurrent state
```

**What was removed:** `_SelfAttentionAmongLatents` (the Q/K/V over the 4 latents that previously followed the cross-attention block in each round). The cross_attn block remains unchanged; output_norm remains unchanged.
...
**Risk introduced by removing SA:** Without self-attn among latents, the 4 latents have **no direct interaction channel** within a single resampler call. They only differ because they begin from 4 distinct placeholder vectors and attend to the same 16-token context with different query projections. If the cross-attn block learns near-identical query weights across latents, the 4 outputs may collapse. Monitor `eff_rank`, `latent_pairwise_cossim_buf`, and `H_t·H_{t+1} cossim` for early signs.
...
### 3.1 Change from exp_003_2 (the only architectural diff)

| Change | exp_003_2 | exp_003_4 (this) | Motivation |
|--------|-----------|------------------|------------|
| **Perceiver round structure** | Cross-attn → Self-attn among latents → output | Cross-attn → output (no SA) | Test whether self-attn among 4 latents is load-bearing for representational diversity. Cross-attn already pools the same 16-patch context for every latent, so SA may be redundant — but removing it eliminates the only intra-latent interaction channel, so it may also drive collapse. |
| **Monitoring** | Tracks `sec3/latent_self_attn_row_jsd_r{i}`, `gnorm_perc_self_r{i}`, `uwr_perc_self_r{i}`, `perceiver_self_attn_r{i}` heatmaps | All removed (no SA to probe) | Smaller monitor surface; collapse detection now relies on `eff_rank`, latent pairwise cossim, and `H_t·H_{t+1}` cossim. |
| **Parameter count** | ~966k encoder params | ~924k encoder params (≈42k fewer; one SA block × 2 rounds) | Fewer params means a slightly easier optimization problem if SA was just noise; harder if SA was load-bearing. |

### 3.2 Inherited changes from exp_003_2 vs earlier experiments

| Change | exp_003_0 | exp_003_1 | exp_003_2 / exp_003_4 | Motivation |
|--------|-----------|-----------|-----------------------|------------|
...
Because exp_003_4 removed `_SelfAttentionAmongLatents`, the following metrics inherited from exp_003_2 **no longer exist** and have been stripped from `monitors/`, `writer.py`, and `panel.js`:

- `sec3/latent_self_attn_row_jsd_r{i}` — there is no latent SA matrix to compute JSD over.
- `gnorm_perc_self_r{i}_*` — the `perc_self_r{i}` sub-block group is gone from gradient decomposition.
- `uwr_perc_self_r{i}_*` — likewise dropped from the update/weight ratio bucket.
- `perceiver_self_attn_r{i}` heatmaps in `debug_runner.py` JSON output and `panel.js`.
- `perc_r{i}_self_ffn` activation-dead-frac hooks.

Collapse detection now relies on the existing latent-space metrics, which are the primary signal for this experiment:

- `sec1/eff_rank` — effective rank of the 4-latent codebook over a buffer batch.
- `sec1/latent_pairwise_cossim_buf` and `sec1/latent_pairwise_cossim_t{1,10,20}` — pairwise cosine similarity among the 4 latents at fixed rollout steps.
- `sec1/ht_htp1_cossim_rollout` and `sec1/ht_htp1_cossim_eval` — adjacent-frame latent cosine similarity (collapse indicator).
- `sec4/action_pred_entropy_eval` — if the encoder collapses, the action predictor cannot distinguish actions and its entropy stays near `log(4) ≈ 1.39`.

The §1.3 round-0 post-attention probe still fires, but the forward hook now targets `cross_attn` (see `monitors/eval_pass.py`) since there is no longer a `self_attn` block to hook in round 0.
...
```
exp_003_4_no_resampler_self_attn/
├── system_card.md              — this document
├── metrics.md                  — Health-metric definitions (sec3 removed; see §7)
...
├── analyze_cross_attn.py       — Post-hoc cross-attn analysis
├── analyze_cross_attn_t0_full.py — t=0 full-bandwidth cross-attn analysis
├── panel.js                    — Dashboard visualization plugin (no SA heatmaps)
├── models/
│   ├── __init__.py             — load_models() factory exposing
│   │                             encoder, state_predictor, action_predictor, policy
│   ├── encoder.py              — LOCAL copy of exp_003_0 encoder with
│   │                             `_SelfAttentionAmongLatents` removed
│   ├── state_predictor.py      — FlowMatchingPredictor
│   ├── action_predictor.py     — ActionPredictor MLP with cross-entropy head
│   └── policy.py               — PolicyNetwork, REINFORCEBaseline
├── monitors/                   — health, attention, gradients, predictors,
│                                  representation, exploration, eval_pass, writer
...
# ── Training (fresh) ────────────────────────────────────────────────────────
cd "Code Repo"
uv run python -m JEPA.experiments.exp_003_4_no_resampler_self_attn.train

# ── Training (resume from checkpoint) ──────────────────────────────────────
uv run python -m JEPA.experiments.exp_003_4_no_resampler_self_attn.train \
    --resume JEPA/experiments/exp_003_4_no_resampler_self_attn/checkpoints/step_050000.pt
...
# ── Evaluation ──────────────────────────────────────────────────────────────
uv run python -m JEPA.experiments.exp_003_4_no_resampler_self_attn.eval \
    --checkpoint checkpoints/step_050000.pt --episodes 20

# ── Debug runner (standalone) ───────────────────────────────────────────────
uv run python JEPA/experiments/exp_003_4_no_resampler_self_attn/debug_runner.py \
    JEPA/experiments/exp_003_4_no_resampler_self_attn/checkpoints/step_050000.pt

# ── Dashboard ───────────────────────────────────────────────────────────────
uv run python JEPA/dashboard/server.py
# Open http://localhost:8787
# 1. Select "exp_003_4_no_resampler_self_attn" in the experiment dropdown
# 2. Select a run from the run dropdown (auto-selects the latest with metrics)
# 3. Charts update every 3 s while training runs
```
...
| Dimension | exp_003_0 | exp_003_1 | exp_003_2 | exp_003_4 (this) |
|-----------|-----------|-----------|-----------|------------------|
| **Resampler self-attn among latents** | Yes | Yes | Yes | **No** |
| **Collapse prevention** | Stop-grad on `h_{t+1}` target | Stop-grad + EMA target encoder | Action Predictor (no EMA) | Action Predictor (no EMA) |
| **Predictors** | State only | State only | State + Action | State + Action |
| **JEPA loss** | `L_state` | `L_state` | `0.5·L_state + 0.5·L_action` | `0.5·L_state + 0.5·L_action` |
| **Buffer target field** | `h_target` (frozen) | `h_target` (frozen) | `next_frame` (re-encoded) | `next_frame` (re-encoded) |
| **Buffer sampling** | 20% recent / 80% uniform | 20% recent / 80% uniform | Uniform only | Uniform only |
| **Encoder gradient paths into JEPA loss** | 1 | 1 | 3 | 3 |
| **EMA target encoder** | — | Cosine 0.996 → 0.9999 | — | — |
| **Max-episode-length guard** | `MAX_EP_STEPS = 300` | `MAX_EP_STEPS = 300` | — (energy-bounded) | — (energy-bounded) |
| **Approx. encoder params** | ~966k | ~966k | ~966k | **~924k** |
