"""
exp_004_1 training loop — LS20 + TU93 + RE86 + G50T shared-encoder JEPA.

Architecture: encoder + Perceiver Resampler (cross-attn only) + state predictor +
action predictor — all SHARED across all four envs. Action embeddings and
policies are PER-ENV. JEPA loss is exp_003_2's `0.5·L_state + 0.5·L_action`;
reward is exp_003_3's state-only curiosity (action term retained in JEPA loss
only). Action space is the max over envs (5); 4-action envs (LS20, TU93) mask
index 4 via `available_actions` at the policy.

Data collection: round-robin episodes (one full episode per env per cycle of 4).
JEPA updates: balanced batches with `batch_size // 4` transitions from each
per-env buffer (n_envs=4 → 16 per env at batch_size=64).
Policy updates: per-env REINFORCE with EMA baseline.

Bug fix (exp_004_0/system_card.md §3.3, inherited): the dying step is excluded
from replay buffer (existing `[:-1]` slice), policy buffer, and rollout health
metrics.

Run:
    cd "Code Repo"
    uv run python -m JEPA.experiments.exp_004_1_four_envs.train
    uv run python -m JEPA.experiments.exp_004_1_four_envs.train --resume <ckpt>
    uv run python -m JEPA.experiments.exp_004_1_four_envs.train --max-steps 5000
"""
...
from JEPA.experiments.exp_004_1_four_envs.config import Config
from JEPA.experiments.exp_004_1_four_envs.models import load_models
from JEPA.experiments.exp_004_1_four_envs.reward_shaping import is_end_of_life

# Reuse exp_003_4's monitor infrastructure verbatim — the encoder/predictor
# architecture is identical, so all sec1-sec7 metrics about the shared modules
# transfer directly. Per-env breakdowns are layered on top.
...
def setup_run_logger(resume_path: str | None) -> tuple[Path, _Tee]:
    ...
    sys.stdout = tee
    print(f"[exp004_1] Run dir:  {run_dir}")
    print(f"[exp004_1] Log file: {log_path}")
    return run_dir, tee
...
def run_one_episode(
    *,
    ...
    # Per-env action-space size. 4 for LS20/TU93; 5 for RE86/G50T.
    env_n_actions = env.n_actions
    ...
    while in_episode and global_step_box[0] < cfg.max_steps:
        ...
        if global_step_box[0] < cfg.warmup_steps:
            # Sample only from the env's own action set — never index ≥ env_n_actions.
            # Guarantees LS20/TU93 do not sample the 5-way head's extra class.
            action_idx = int(np.random.randint(0, env_n_actions))
        ...
        # Safety assertion — surfaces broken available_actions masking immediately.
        assert action_idx < env_n_actions, (
            f"action_idx={action_idx} ≥ env.n_actions={env_n_actions} in {env_name!r}"
        )
        ...
        # See exp_004_0/system_card.md §3.3 (inherited).
        if not life_end:
            with torch.no_grad():
...
def jepa_update(
    *,
    ...
    assert cfg.batch_size % n_envs == 0, (
        f"batch_size={cfg.batch_size} must be divisible by n_envs={n_envs}"
    )
    ...
    L_state  = sum(L_state_per_env.values())  / n_envs
    ...
    for e in env_names:
        clip_params += list(action_embeds[e].parameters())
    nn.utils.clip_grad_norm_(clip_params, cfg.grad_clip_model)
    ...
    per_lat_state_avg = per_lat_state_combined / n_envs
...
def train(cfg: Config, resume_path: str = None, run_dir: Path = None,
    ...
    n_envs = len(env_names)
    print(f"[exp004_1] device={device}  max_steps={cfg.max_steps}")
    print(f"[exp004_1] envs={env_names}  game_ids={list(cfg.game_ids)}")
    print(f"[exp004_1] lambdas: state={cfg.lambda_state}  action={cfg.lambda_action}")
    print(f"[exp004_1] reward weights: state={cfg.reward_w_state}  action={cfg.reward_w_action}  "
          f"clamp={cfg.reward_clamp}")
    print(f"[exp004_1] buffer_size_per_env={cfg.buffer_size_per_env}  n_envs={n_envs}")
    print(f"[exp004_1] batch_size={cfg.batch_size}  (per-env share = {cfg.batch_size // n_envs})")
    assert cfg.batch_size % n_envs == 0, (
        f"batch_size={cfg.batch_size} must be divisible by n_envs={n_envs}"
    )
    ...
    if resume_path is not None:
        ...
        print(f"[exp004_1] Resumed from step {start_step}")
    ...
    if run_dir is not None:
        ...
        if not ph_init_path.exists():
            ...
            print(f"[exp004_1] Cached placeholder init → {ph_init_path}")
    ...
    # All per-env action-embedding tables go into state_pred_opt (their gradients
    # flow through L_state only — the action predictor doesn't read them).
    ...
    # Sub-block parameter partition (used by gnorm totals). Use the first env's
    # action embedding + policy for partition compatibility — the shared
    # modules' gnorms are still tracked under the standard sub_blocks names.
    ...
    for name, gid in zip(env_names, cfg.game_ids):
        ...
        print(f"[exp004_1] Loaded env '{name}' ({gid}) — n_actions={envs[name].n_actions}")
    ...
    jepa_updates = 0
    next_probe_at = cfg.capture_attn_freq
    global_step_box = [start_step]
    t0 = time.time()

    print(f"\n[exp004_1] Training started  warmup={cfg.warmup_steps}  seed={cfg.seed}\n")
    ...
    while global_step_box[0] < cfg.max_steps:
        ...
        if buf_min_satisfied:
            ...
            # on a small balanced peek over all 4 envs.
            if jepa_updates >= next_probe_at:
                next_probe_at = jepa_updates + cfg.capture_attn_freq
                with torch.no_grad():
                    min_buf = min(len(latent_bufs[e]) for e in env_names)
                    per_env_share = max(1, min(4, min_buf // 2))
                    mbs = [latent_bufs[e].sample(per_env_share, device)
                           for e in env_names]
                    mb_frames = torch.cat([m.frames for m in mbs], dim=0)
        ...
        if step // LOG_FREQ != (step - steps_done) // LOG_FREQ:
            ...
            for c in criticals:
                print(f"  [CRITICAL] {c}")
            if criticals:
                print("[exp004_1] Critical issue — saving checkpoint and stopping")
    ...
    print(f"\n[exp004_1] Training complete at step {global_step_box[0]}. "
...
def main():
    ...
    parser.add_argument(
        ...
        help="Directory to write checkpoints into. Defaults to "
             "exp_004_1_four_envs/checkpoints/.",
    )
