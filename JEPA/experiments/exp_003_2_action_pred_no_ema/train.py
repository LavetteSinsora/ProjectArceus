"""
Exp-003-2 training loop — Action Predictor JEPA (no EMA target).
...
Run:
    ...
    uv run python -m JEPA.experiments.exp_003_2_action_pred_no_ema.train --resume <ckpt>
"""
...
from JEPA.experiments.exp_003_2_action_pred_no_ema.monitors.health import HealthMonitor
from JEPA.experiments.exp_003_2_action_pred_no_ema.monitors.writer import MetricsWriter
from JEPA.experiments.exp_003_2_action_pred_no_ema.monitors import (
...
def setup_run_logger(resume_path: str | None) -> tuple[Path, _Tee]:
    ...
    sys.stdout = tee
    print(f"[exp003_2] Run dir:  {run_dir}")
    print(f"[exp003_2] Log file: {log_path}")
    return run_dir, tee
...
def train(cfg: Config, resume_path: str = None, run_dir: Path = None,
    ...
    print(f"[exp003_2] device={device}  max_steps={cfg.max_steps}")
    print(f"[exp003_2] lambdas: state={cfg.lambda_state}  action={cfg.lambda_action}")
    print(f"[exp003_2] reward weights: state={cfg.reward_w_state}  action={cfg.reward_w_action}  "
          f"clamp={cfg.reward_clamp}")
    print(f"[exp003_2] checkpoint_schedule={checkpoint_schedule}")
    if checkpoint_dir is not None:
        print(f"[exp003_2] checkpoint_dir={checkpoint_dir}")
    ...
    if resume_path is not None:
        ...
        print(f"[exp003_2] Resumed from step {start_step}")
    ...
    if run_dir is not None:
        ...
        if not ph_init_path.exists():
            ...
            print(f"[exp003_2] Cached placeholder init → {ph_init_path}")
    ...
    print(f"\n[exp003_2] Training started  warmup={cfg.warmup_steps}  seed={cfg.seed}\n")
    ...
    while step < cfg.max_steps:
        ...
        if step % cfg.update_freq == 0 and len(latent_buf) >= cfg.min_buffer_size:
            ...
            if jepa_updates % cfg.grad_decomp_freq == 0:
                try:
                    ...
                    print(f"[exp003_2] grad decomposition failed at step {step}: {e}")
        ...
        if step > start_step and step % cfg.eval_freq == 0:
            try:
                ...
                print(f"[exp003_2] eval pass failed at step {step}: {e}")
        ...
        if step % LOG_FREQ == 0 and step > 0:
            ...
            for c in criticals:
                print(f"  [CRITICAL] {c}")
            if criticals:
                print("[exp003_2] Critical issue — saving checkpoint and stopping")
    ...
    path = save_checkpoint(step, encoder, state_predictor, action_predictor,
                           ...
                           ckpt_dir=checkpoint_dir)
    print(f"\n[exp003_2] Training complete at step {step}. Final checkpoint: {path.name}")
    act_mon.remove()
    if metrics_writer is not None:
...
def main():
    ...
    parser.add_argument(
        ...
        help=("Directory to write checkpoints into. Defaults to "
              "exp_003_2_action_pred_no_ema/checkpoints/. Use this to keep "
