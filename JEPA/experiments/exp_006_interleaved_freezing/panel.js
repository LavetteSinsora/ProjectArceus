...
// ════════════════════════════════════════════════════════════════════════════
// SECTION E — ACTION PREDICTOR
//   Shows the trained action_predictor(h_t, h_{t+1}) head at this checkpoint:
//   the two latent groups are stacked vertically as row-vector heatmaps and
//   transformed into a probability distribution over actions, with the
//   highest-probability action highlighted. Lets us watch whether the
//   anti-collapse predictor actually distinguishes states or just outputs a
//   near-uniform / locked distribution.
// ════════════════════════════════════════════════════════════════════════════

const AP_DIM_PX  = 2;    // px per dim (4 latents × 128 dims = 256 px wide)
const AP_ROW_H   = 12;   // px per latent-row height
const AP_ROW_GAP = 1;
const AP_BLOCK_GAP = 6;  // px between the H_t and H_{t+1} blocks

let _apStep = null;
let _apTipEl = null;

function _drawApHBlock(canvas, vecs) {
  // vecs: array of 4 latents, each a 128-length vector. Drawn as 4 row-vector
  // strips (latent index top→bottom, dim index left→right) using the shared
  // emb colour map.
  const W = 128 * AP_DIM_PX;
  const H = vecs.length * AP_ROW_H + (vecs.length - 1) * AP_ROW_GAP;
  canvas.width = W; canvas.height = H;
  const ctx = canvas.getContext('2d');
  const img = ctx.createImageData(W, H);
  for (let i = 0; i < img.data.length; i += 4) {
  ...
  for (let b = 0; b < vecs.length; b++) {
    const vec = vecs[b];
    const y0 = b * (AP_ROW_H + AP_ROW_GAP);
    for (let d = 0; d < 128; d++) {
      const [r, g, bv] = window.JEPA.embRgb(vec[d] ?? 0);
      const x0 = d * AP_DIM_PX;
      for (let py = y0; py < y0 + AP_ROW_H; py++) {
        for (let px = x0; px < x0 + AP_DIM_PX; px++) {
          const base = (py * W + px) * 4;
          img.data[base]=r; img.data[base+1]=g; img.data[base+2]=bv; img.data[base+3]=255;
        }
      }
...
function _drawApProbBars(canvas, probs, predicted, taken) {
  // Right-hand "prob over actions" view. The argmax (predictor's prediction)
  // is filled gold + star; the sampled-by-policy action gets a small ▼ marker
  ...
  for (let i = 0; i < n; i++) {
    ...
    ctx.textBaseline = 'middle'; ctx.textAlign = 'left';
    ctx.fillText(`A${i}`, 2, y + barH / 2);
    const barX = 24, barW = w - barX - 56;
    ctx.fillStyle = '#161b22'; ctx.fillRect(barX, y, barW, barH);
    const fill = Math.max(1, Math.round(probs[i] * barW));
    ctx.fillStyle = i === predicted ? '#f0c040' : colors[i % colors.length];
    ctx.fillRect(barX, y, fill, barH);
    if (i === predicted) {
      ctx.fillStyle = '#fff'; ctx.font = 'bold 10px monospace';
      ctx.textBaseline = 'middle'; ctx.textAlign = 'left';
      ctx.fillText('★', barX + fill + 3, y + barH / 2);
    }
    if (i === taken && taken !== predicted) {
      ctx.fillStyle = '#9fb6c8'; ctx.font = 'bold 11px monospace';
      ctx.textBaseline = 'middle'; ctx.textAlign = 'left';
      ctx.fillText('▼', barX + fill + 3, y + barH / 2);
    }
    ctx.fillStyle = TEXT; ctx.font = '9px monospace';
    ctx.textBaseline = 'middle'; ctx.textAlign = 'right';
    ctx.fillText((probs[i] * 100).toFixed(1) + '%', w - 2, y + barH / 2);
  }
}

function _apAttachHover(canvas, vecs, blockLabel) {
  if (!_apTipEl) {
    _apTipEl = _el('div', {}, {
      position: 'fixed', display: 'none', zIndex: '9999',
      background: '#1c2128', border: `1px solid ${BORDER}`,
    ...
    document.body.appendChild(_apTipEl);
  }
  const tip = _apTipEl;
  canvas.onmousemove = (e) => {
    const rect = canvas.getBoundingClientRect();
    const mx = (e.clientX - rect.left) * (canvas.width  / rect.width);
    const my = (e.clientY - rect.top)  * (canvas.height / rect.height);
    const latIdx = Math.min(vecs.length - 1, Math.floor(my / (AP_ROW_H + AP_ROW_GAP)));
    const dimIdx = Math.min(127, Math.floor(mx / AP_DIM_PX));
    const vec = vecs[latIdx];
    if (!vec) { tip.style.display = 'none'; return; }
    const norm = Math.sqrt(vec.reduce((s, v) => s + v * v, 0));
    tip.innerHTML =
      `<b style="color:${TEXT_HI}">${blockLabel} — L${latIdx}</b><br>` +
      `dim <b>${dimIdx}</b>: <b>${Number(vec[dimIdx]).toFixed(4)}</b><br>` +
      `norm=<b>${norm.toFixed(3)}</b>`;
    tip.style.display = 'block';
    let x = e.clientX + 12, y2 = e.clientY + 12;
...
function _buildActionPredictorSection(el) {
  const wrap = _el('div');

  const hint = _el('div', {}, { fontSize: '10px', color: TEXT_DIM, marginBottom: '8px' });
  hint.innerHTML =
    `(h_t, h_{t+1}) → action_predictor → softmax. ` +
    `<span style="color:#f0c040">★</span>=argmax, ` +
    `<span style="color:#9fb6c8">▼</span>=actually-taken (if differs).`;
  wrap.appendChild(hint);
  ...
  // ── Left: H_t / H_{t+1} stacked as row-vector heatmaps ─────────────────
  const hCol = _el('div', {}, { display: 'flex', flexDirection: 'column', gap: AP_BLOCK_GAP + 'px' });
  [
    ['p03-ap-ht',   'H_t  (4 × 128)'],
    ['p03-ap-htp1', 'H_{t+1}  (4 × 128)'],
  ].forEach(([id, label]) => {
    const block = _el('div', {}, { display: 'flex', flexDirection: 'column', alignItems: 'flex-start' });
    const hdr = _el('div', {}, {
      fontSize: '9px', color: TEXT_HI, fontWeight: 'bold', marginBottom: '3px',
      letterSpacing: '.5px',
    });
    hdr.textContent = label;
    block.appendChild(hdr);
    const c = _el('canvas', { id }, {
      display: 'block', imageRendering: 'pixelated',
      border: `1px solid ${BORDER}`, cursor: 'crosshair',
    });
    block.appendChild(c);
    hCol.appendChild(block);
  });
  row.appendChild(hCol);

  // ── Arrow ─────────────────────────────────────────────────────────────
  const arrow = _el('div', {}, {
    alignSelf: 'center', fontSize: '20px', color: ACCENT,
    fontFamily: "'Menlo','Monaco','Consolas',monospace",
  });
  arrow.textContent = '→';
  row.appendChild(arrow);

  // ── Right: action-probability bars + meta ──────────────────────────────
  const probsCol = _el('div', {}, { display: 'flex', flexDirection: 'column' });
  const probHdr = _el('div', {}, {
    fontSize: '9px', color: TEXT_HI, fontWeight: 'bold', marginBottom: '3px',
    letterSpacing: '.5px',
  });
  probHdr.textContent = 'P(action | h_t, h_{t+1})';
  probsCol.appendChild(probHdr);
  const pc = _el('canvas', { id: 'p03-ap-probs', width: '240', height: '132' },
    { display: 'block' });
  probsCol.appendChild(pc);
  const meta = _el('div', {}, {
    marginTop: '6px', fontSize: '11px', color: TEXT_DIM, display: 'flex',
    flexDirection: 'column', gap: '2px', fontVariantNumeric: 'tabular-nums',
  });
  meta.innerHTML =
    `Predicted <span id="p03-ap-pred" style="color:${TEXT_HI}">—</span> ` +
    `<span style="color:#555">vs</span> Taken <span id="p03-ap-taken" style="color:${TEXT_HI}">—</span><br>` +
    `Entropy <span id="p03-ap-ent" style="color:${TEXT_HI}">—</span> ` +
    `<span style="color:${TEXT_DIM}">/ ln4=1.386</span><br>` +
    `CE vs taken <span id="p03-ap-ce" style="color:${TEXT_HI}">—</span>`;
  probsCol.appendChild(meta);
  row.appendChild(probsCol);

  wrap.appendChild(row);
  el.appendChild(_section('ACTION PREDICTOR', wrap));
}

function _refreshActionPredictorPanel() {
  if (!_apStep) return;
  const step = _apStep;
  const ht   = step.latent_vectors;        // (4, 128) = h_t
  const htp1 = step.h_target_latents;      // (4, 128) = h_{t+1}
  const probs     = step.action_predictor_probs;
  const predicted = step.action_predictor_predicted;
  const taken     = step.action_taken;
  const entropy   = step.action_predictor_entropy;
  const ce        = step.action_predictor_ce;

  const cHt   = document.getElementById('p03-ap-ht');
  const cHtp1 = document.getElementById('p03-ap-htp1');
  if (cHt   && ht)   { _drawApHBlock(cHt,   ht);   _apAttachHover(cHt,   ht,   'h_t');     }
  if (cHtp1 && htp1) { _drawApHBlock(cHtp1, htp1); _apAttachHover(cHtp1, htp1, 'h_{t+1}'); }

  const pc = document.getElementById('p03-ap-probs');
  if (pc && probs) _drawApProbBars(pc, probs, predicted, taken);

  const fmt = (v, dp=4) => v != null ? Number(v).toFixed(dp) : '—';
  const pe = document.getElementById('p03-ap-pred');
  const te = document.getElementById('p03-ap-taken');
  const ee = document.getElementById('p03-ap-ent');
  const ce_ = document.getElementById('p03-ap-ce');
  if (pe) pe.innerHTML  = predicted != null ? `A${predicted} ★` : '—';
  if (te) te.innerHTML  = taken != null     ? `A${taken}` + (taken === predicted ? ' ✓' : ' ✗') : '—';
  if (ee) ee.textContent = fmt(entropy);
  if (ce_) ce_.textContent = fmt(ce);
}
...
export function render(data, el) {
  ...
  _saLayer    = 1;
  _selSAPatch = null;
  _saStep = _percStep = _predStep = _apStep = null;
  ...
  _buildSASection(el);
  _buildActionPredictorSection(el);
