...
Run:
    cd "Code Repo"
    uv run python -m JEPA.experiments.exp_003_4_no_resampler_self_attn.analyze_cross_attn \\
        --checkpoint JEPA/experiments/exp_003_4_no_resampler_self_attn/checkpoints/step_015000.pt \\
        --max-steps 40
"""
...
def analyze_step(encoder, queries: torch.Tensor, frame_t: torch.Tensor) -> dict:
    ...
    for r_idx, round_block in enumerate(encoder.perceiver.rounds):
        ...
        h = ca.ffn(h)
        # exp_003_4: no self-attention among latents; round output is ffn(cross_attn).
