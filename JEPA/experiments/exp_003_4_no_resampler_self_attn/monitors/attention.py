"""
Attention probes (metrics.md §2).

exp_003_4 has no self-attention among latents, so only patch-level SA
attention is captured. The encoder caches post-softmax attention into
`module._debug_attn` only when `module.training` is False. For training-time
probes we briefly flip the encoder into eval() mode, run a small forward,
read the cached tensors, and restore training() mode.
"""
