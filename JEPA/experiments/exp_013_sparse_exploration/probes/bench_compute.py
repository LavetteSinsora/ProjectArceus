...
It sweeps n_envs ∈ {8,16,32,64} for two representative methods:
    exp_013_1_rnd_icm  — the shared PPO+ICM+RND shape (methods 1/2/4 are ~this)
    exp_013_5_lookahead — the heaviest: A forward-model evals PER env step

Usage:
    uv run python -m JEPA.experiments.exp_013_sparse_exploration.probes.bench_compute \
...
def bench_lookahead(n_envs, updates, warmup, rollout_steps, device):
    ...
    from JEPA.experiments.exp_013_sparse_exploration.exp_013_5_lookahead.config import Config
    from JEPA.experiments.exp_013_sparse_exploration.exp_013_5_lookahead.lookahead import (
