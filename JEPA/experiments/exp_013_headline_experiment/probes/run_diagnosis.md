...
- **A** = `exp_013_1b_leaky_rnd_on_icm_phi`, `phi_mode=frozen` (frozen-φ + RND-on-φ)
- **B** = `exp_013_1b_leaky_rnd_on_icm_phi`, `phi_mode=icm` (trainable φ via ICM inverse model + RND-on-φ)
- **C** = `exp_013_2_additive_rnd_icm` (`rnd_icm_additive`, ICM-novelty + RND-on-φ summed, w_icm=0.5)
- **D** = `exp_013_3_mcts_lookahead` (`lookahead_mcts`, actor-free depth-1 Boltzmann over Q = novelty + γV̂)
- **E** = `exp_013_4_plan2explore` — **no real runs on disk** (only a 256-step smoke; see below).
