...
### 1.3 `round0_postSA_pairwise_cossim @ t=1`  *(primary collapse mechanism)*
- **What:** Mean pairwise cosine similarity between the 4 latent tokens **immediately after the self-attention sub-block of round 0** in the Perceiver Resampler, at `t=1` (placeholder queries).
- **How:** Hook the output of `encoder.perceiver.rounds[0].self_attn` (the `_SelfAttentionAmongLatents` module) during an eval forward pass. The hooked tensor has shape `(B, 4, D)`. Compute pairwise cosine on each batch element, average.
- **Why:** This is the exact location previously identified as where the representation collapses — the SA-among-latents step inside round 0 averages latents toward a single direction. Watching this point is more diagnostic than watching the final `H_t`.
- **Soundness:** Requires a forward hook (e.g., `register_forward_hook`) since the post-SA tensor is not currently returned. The existing `_debug_attn` attribute is only populated in `model.eval()` mode (see [encoder.py:158](JEPA/experiments/exp_003_0_normalized_latent_jepa/models/encoder.py#L158-L159)) — store the post-SA activation similarly via a hook to avoid mutating forward signatures.
...
### 3.1 `latent_self_attn_row_jsd`
- **What:** Pairwise JSD between the 4 rows of the self-attention matrix inside `_SelfAttentionAmongLatents`. One scalar per perceiver round.
- **How:** Hook `attn_w` (shape `(B, n_heads, 4, 4)`) inside `_SelfAttentionAmongLatents.forward` ([encoder.py:157](JEPA/experiments/exp_003_0_normalized_latent_jepa/models/encoder.py#L157)). For each head, compute `pairwise_jsd(attn_w[h])`. Average across heads → per-round scalar. Average over eval steps and episodes.
- **Why:** This is the precise mechanism that gives 1.3 its name. If the 4 query-rows in latent SA are identical distributions, each latent computes the same weighted average of the values — guaranteed collapse on output. JSD must be strictly > 0 and ideally stable.
- **Soundness:** As in 2.1, `_debug_attn` is captured only in eval mode at [encoder.py:159](JEPA/experiments/exp_003_0_normalized_latent_jepa/models/encoder.py#L159). For training-mode capture, add a `capture_attn` flag and rely on it during the metric pass.

### 3.2 `grad_norm_perceiver_{cross,self}`
- **What:** L2 norm of gradients flowing into (a) all cross-attn params and (b) all latent-SA params within `encoder.perceiver`.
- **How:** Partition `encoder.perceiver.parameters()` into the two groups before training:
  ```python
  cross_params = [p for r in encoder.perceiver.rounds for p in r.cross_attn.parameters()]
  latent_sa_params = [p for r in encoder.perceiver.rounds for p in r.self_attn.parameters()]
  ```
  After `backward()`, compute `grad_norm(cross_params)` and `grad_norm(latent_sa_params)` per step.
- **Why:** The current code only logs `grad_enc_s2 = grad_norm(perceiver.parameters())` — a single scalar mixing cross-attn, latent SA, FFNs, output_norm, and placeholders. Splitting it isolates which sub-mechanism is being trained hard; the system_card already attributes the 5→17 grad growth in exp_002 to the cross/self interaction.
- **Soundness:** The existing `health.grad_enc_s2` ([train.py:436](JEPA/experiments/exp_003_1_ema_target/train.py#L436)) should be kept as a coarse total. Add these two as a finer-grained breakdown rather than replacing it.
...
### 7.2 Encoder — Perceiver Resampler (split into 5 sub-blocks × 3 sources + total)

For each sub-block below, log 4 scalars: `[from_state]`, `[from_action_via_Ht]`, `[from_action_via_Htp1]`, `[total]`.

| Sub-block | Params covered |
|---|---|
| `gnorm_perc_cross_r0[*]` | `encoder.perceiver.rounds[0].cross_attn.*` |
| `gnorm_perc_self_r0[*]`  | `encoder.perceiver.rounds[0].self_attn.*`  |
| `gnorm_perc_cross_r1[*]` | `encoder.perceiver.rounds[1].cross_attn.*` |
| `gnorm_perc_self_r1[*]`  | `encoder.perceiver.rounds[1].self_attn.*`  |
...
For each upstream parameter group `G` in §7.1 + §7.2 (patch SA, perc_cross_r0, perc_self_r0, perc_cross_r1, perc_self_r1, perc_other):
...
| `uwr_perc_self_r0`  | `encoder.perceiver.rounds[0].self_attn.*`  |
| `uwr_perc_cross_r1` | `encoder.perceiver.rounds[1].cross_attn.*` |
| `uwr_perc_self_r1`  | `encoder.perceiver.rounds[1].self_attn.*`  |
...
1. **Hook points.** Add `register_forward_hook` to: (a) `encoder.perceiver.rounds[0].self_attn` (captures round-0 post-SA latent for 1.3); (b) `encoder.sa_blocks[*]` and `encoder.perceiver.rounds[*].self_attn` for attention matrices (sections 2.1, 2.2, 3.1) — store as tensors on a `metrics_state` dict guarded by a `capture_attn = True` flag set only during the metric pass.
